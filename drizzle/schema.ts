...
/**
 * Candidate News - news articles specific to each candidate
 * Auto-updated daily via Gemini API search
 */
export const candidateNews = mysqlTable("candidate_news", {
  id: int("id").autoincrement().primaryKey(),
  candidateId: int("candidateId").notNull(),
  title: varchar("title", { length: 300 }).notNull(),
  summary: text("summary"),
  ...
  sourceName: varchar("sourceName", { length: 100 }),
  imageUrl: text("imageUrl"),
  topic: varchar("topic", { length: 100 }), // 議題分類，用於去重
  publishedAt: timestamp("publishedAt").defaultNow().notNull(),
  searchedAt: timestamp("searchedAt").defaultNow().notNull(), // 搜尋時間
  isAutoGenerated: boolean("isAutoGenerated").default(true), // 是否由 AI 自動生成
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type CandidateNews = typeof candidateNews.$inferSelect;
export type InsertCandidateNews = typeof candidateNews.$inferInsert;
