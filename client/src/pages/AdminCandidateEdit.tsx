    ...
    <div className="min-h-screen bg-background">
      ...
      <Dialog open={isNewsSearchDialogOpen} onOpenChange={setIsNewsSearchDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>搜尋結果 - AI 已產生 {newsSearchResults.length} 則新聞</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {newsSearchResults.map((item, index) => (
              <Card key={index} className="hover:bg-accent/50 transition-colors border-l-4 border-l-primary">
                <CardContent className="p-4">
                  <div className="space-y-3">
                    {/* Header: Date and Source */}
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      {item.publishedDate && (
                        <span className="font-medium">{item.publishedDate}</span>
                      )}
                        ...
                        onClick={async () => {
                          try {
                            await createNews.mutateAsync({
                              candidateId,
                              title: item.title,
                              summary: item.summary, // Use the AI-generated summary
          ...
          <DialogFooter className="flex gap-2">
            {newsSearchResults.length > 0 && (
              <Button
                ...
                onClick={async () => {
                  try {
                    toast.info(`正在新增 ${newsSearchResults.length} 則新聞...`);
                    for (const item of newsSearchResults) {
                      await createNews.mutateAsync({
                        candidateId,
                        title: item.title,
                    ...
                    toast.success(`已新增 ${newsSearchResults.length} 則新聞`);
                    setIsNewsSearchDialogOpen(false);
                  } catch (e: any) {
                    toast.error(e.message || "新增失敗");
                  }
                }}
